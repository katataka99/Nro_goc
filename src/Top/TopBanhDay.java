 ...
 * @author Admin
 */
public class TopBanhDay {

    @Getter
    private List<Player> list = new ArrayList<>();
    private static final TopBanhDay INSTANCE = new TopBanhDay();

    public static TopBanhDay getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); java.sql.PreparedStatement ps = con.prepareStatement(
                "SELECT *, "
                + "CAST(JSON_EXTRACT(data_tet, '$[2]') AS UNSIGNED) AS banhDay "
                + "FROM player "
                + "ORDER BY banhDay DESC "
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.point_banhday = rs.getInt("banhDay");
