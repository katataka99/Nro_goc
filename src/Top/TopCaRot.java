 ...
 * @author Admin
 */
public class TopCaRot {

    @Getter
    private final List<Player> list = new ArrayList<>();
    private static final TopCaRot INSTANCE = new TopCaRot();

    public static TopCaRot getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        String sql = """
                    SELECT p.id, p.name, p.head, p.gender, p.data_inventory,
                           p.data_point, p.items_body, a.carot
                    FROM player p
                    JOIN account a ON p.account_id = a.id
                    WHERE a.carot IS NOT NULL
                    ORDER BY a.carot DESC
                    LIMIT 100
                """;
