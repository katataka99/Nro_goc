 ...
 * @author Admin
 */
public class TopGioTo {

    @Getter
    private List<Player> list = new ArrayList<>();
    private static final TopGioTo INSTANCE = new TopGioTo();

    public static TopGioTo getInstance() {
        return INSTANCE;
    }
    ...
    public void load() {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); java.sql.PreparedStatement ps = con.prepareStatement(
                "SELECT *, "
                + "CAST(JSON_EXTRACT(data_tet, '$[1]') AS UNSIGNED) AS gioTo "
                + "FROM player "
                + "ORDER BY gioTo DESC "
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        player.point_gioto = rs.getInt("gioTo");
