...
//    //lv 10 10000 - 57 ngọc
    ...
    public void openMenuTree() {
        ...
        try {
            ...
            if (!isUpgrade) {
                ...
                if (this.currPeas < this.getMaxPea()) {
                    int gemCost = this.level; // Mỗi cấp tương ứng số ngọc kết hạt nhanh
                ...
                int[] gemCosts = {0, 5, 10, 15, 20, 25, 30, 35, 50, 100};
                int level = player.magicTree.level;
                int gemNeed = level < gemCosts.length ? gemCosts[level] : 0;
    ...
    public void unupgradeMagicTree() {
        ...
        Logger.success("Người chs: " + this.player.name + "Vàng nhận từ hủy nâng cấp cây đậu: " + goldReturn);
        PlayerService.gI().sendInfoHpMpMoney(this.player);
        this.isUpgrade = false;
        this.loadMagicTree();
    }
    ...
    public void fastRespawnPea() {
        int gemCost = this.level; // Mỗi cấp 1 ngọc, cấp 1 → 1 ngọc, cấp 10 → 10 ngọc

        if (this.player.inventory.gem < gemCost) {
            Service.gI().sendThongBao(player, "Không đủ ngọc để kết hạt nhanh! Cần " + gemCost + " ngọc.");
            return;
        }

        // Trừ ngọc
        this.player.inventory.gem -= gemCost;
        PlayerService.gI().sendInfoHpMpMoney(this.player); // Cập nhật giao diện ngọc

        // Làm đầy đậu
        this.currPeas = this.getMaxPea();
        this.loadMagicTree();

        Service.gI().sendThongBao(player, "Đã kết hạt nhanh thành công! Đã dùng " + gemCost + " ngọc.");
    }

    public void fastUpgradeMagicTree() {
        // Bảng giá nâng cấp theo từng cấp độ
        int[] gemCosts = {0, 5, 10, 15, 20, 25, 30, 35, 50, 100}; // index 0 không dùng, cấp 1 → index 1

        if (this.level >= MAX_LEVEL) {
            Service.gI().sendThongBao(player, "Cây đậu đã đạt cấp tối đa!");
            return;
        }
        int nextLevel = this.level + 1;
        int gemCost = gemCosts[this.level]; // Vì level hiện tại là lv1 thì chi phí lên lv2
        if (this.player.inventory.gem < gemCost) {
            Service.gI().sendThongBao(player, "Không đủ ngọc để nâng cấp nhanh!");
            return;
        }
        this.player.inventory.gem -= gemCost;
        PlayerService.gI().sendInfoHpMpMoney(this.player); // Cập nhật UI tài nguyên
        this.level = (byte) nextLevel;
        this.isUpgrade = false;
        this.loadMagicTree();

        Service.gI().sendThongBao(player, "Nâng cấp cây đậu nhanh thành công!");
    }
