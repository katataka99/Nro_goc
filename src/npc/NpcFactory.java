    ...
    public static Npc createNPC(int mapId, int status, int cx, int cy, int tempId) {
        ...
        try {
            return switch (tempId) {
                ...
                case ConstNpc.QUAY_HANG_CA_ROT ->
                    new QuayHangCaRot(mapId, status, cx, cy, tempId, avatar);
                default ->
                ...
                switch (player.iDMark.getIndexMenu()) {
                    ...
                    case ConstNpc.BUY_BACK -> {
                        if (select == 0) {
                            Player pl = (Player) PLAYERID_OBJECT.get(player.id);
                            if (!pl.isOffline) {
                                if (player.nPoint.power < 200_000_000L || pl.nPoint.power < 200_000_000) {
                                    Service.gI().sendThongBao(player, "Bạn và đối phương cần có 200tr sức mạnh trở nên để dùng tính năng này!");
                                    return;
                                }
                                if (InventoryService.gI().getCountEmptyBag(player) == 0) {
                                    Service.gI().sendThongBao(player, "Hành trang của bạn ít nhất cần 1 ô trống!");
                                    return;
                                }
                                ItemShop item = ItemService.gI().getItemShop(pl.inventory.itemsBody.get(5).template.id);
                                player.inventory.gem -= (item.cost * 95 / 100);
                                Service.gI().sendMoney(player);
                                InventoryService.gI().addItemBag(player, ItemService.gI().createItemFromItemShop(item));
                                InventoryService.gI().sendItemBag(player);
                                Service.gI().sendThongBao(player, "Bạn nhận được " + item.temp.name);
                                int getCost = item.cost * 20 / 100;
                                pl.inventory.gem += getCost;
                                Service.gI().sendMoney(pl);
                                Service.gI().sendThongBao(pl, "Bạn nhận được " + getCost + " ngọc xanh khi bán lại cải trang");
                            } else {
                                Service.gI().sendThongBao(player, "Người bán đã offline!");
                            }
                        }
                    }
                    case ConstNpc.OTT -> {
                    ...
                    case 671 -> {
                        switch (select) {
                            case 0 -> {
                                ...
                                long[] time = new long[]{900000, 1800000, 3600000, 43200000, 86400000, 259200000,
                                    475200000};
                    ...
                    case ConstNpc.CONFIRM_REMOVE_ALL_ITEM_MAIL_BOX -> {
                        if (select == 0) {
                            ...
                            if (SQLHandle.updateMailBox(player)) {
