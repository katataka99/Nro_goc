        ...
        if (canOpenNpc(pl)) {
            switch (this.mapId) {
                ...
                case 129 -> {
                    ...
                    long gemchallenge = pl.gemChallenge;
                    // Hướng dẫn thêm - Hủy\nđăng kí - về dhvt
            ...
            if (this.mapId == 52) {
                ...
                switch (player.iDMark.getIndexMenu()) {
                    case ConstNpc.BASE_MENU -> {
                        ...
                        long gemchallenge = player.gemChallenge;
                                        ...
                                        if (InventoryService.gI().finditemWoodChest(player)) {
                                            if (select == 1) {
                                                if (player.inventory.gem >= gemchallenge) {
                                                    ...
                                                    player.gemChallenge += 5;
                                                } else {
                                                    Service.gI().sendThongBao(player, "Bạn không đủ ngọc, còn thiếu " + Util.numberToMoney(gemchallenge - player.inventory.gem) + " ngọc nữa");
                                                }
                                            } else {
