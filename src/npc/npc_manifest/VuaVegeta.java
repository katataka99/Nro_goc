...
public class VuaVegeta extends Npc {
    ...
    public void openBaseMenu(Player player) {
        ...
        if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
            ...
            createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, ta rất vui khi gặp được con\nCon muốn làm gì nào ?", menus);
        }
    }
            ...
            case 1 -> {
                if (player.LearnSkill.Time != -1 && player.LearnSkill.Time <= System.currentTimeMillis()) {
                    ...
                    if (player.gender == 2) {
                        ...
                        NpcService.gI().createTutorial(player, tempId, avartar, "Ta chỉ dạy kĩ năng cho người Xayda");
                    }
                }
            }
    ...
    private void openSkillLearningMenu(Player player) {
        if (player.LearnSkill.Time != -1) {
            ...
            createOtherMenu(player, 12, "Con đang học kỹ năng\n" +
    ...
    private void finishLearningSkill(Player player) {
        ...
        try {
            ...
            Skill curSkill = SkillUtil.createSkill(SkillUtil.getTempSkillSkillByItemID(player.LearnSkill.ItemTemplateSkillId), targetLevel);
            ...
            Service.gI().sendThongBao(player, "Bạn đã học xong " + SkillUtil.findSkillTemplate(SkillUtil.getTempSkillSkillByItemID(player.LearnSkill.ItemTemplateSkillId)).name + " cấp " + targetLevel + "!");
        } catch (Exception e) {
