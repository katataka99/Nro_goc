    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            ...
            switch (this.mapId) {
                ...
                case 154 -> this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ta có thể giúp gì cho ngươi ?",
                        "Đến\nhành tinh\nngục tù", "Shop\nOsin", "Từ chối");
                case 155 -> this.createOtherMenu(player, ConstNpc.BASE_MENU, "Ta có thể giúp gì cho ngươi ?",
                        "Quay về", "Từ chối");
                case 52 -> {
                    player.fightMabu.clear();
                    if (TimeUtil.isMabu14HOpen()) {
                        this.createOtherMenu(player, ConstNpc.BASE_MENU,
                                "Mabư hiện tại vẫn đang bị phong ấn\nhãy chờ đến lúc hắn xuất hiện", "OK");
                    } else {
                        this.createOtherMenu(player, ConstNpc.MENU_NOT_OPEN_MMB,
                                "Mabư chưa xuất hiện, hãy quay lại sau.", "OK");
                    }
                }
                ...
                case 127 -> {
                    String npcSay = player.isPhuHoMapMabu
            ...
            switch (this.mapId) {
                case 50 -> {
                    if (player.iDMark.isBaseMenu()) {
                        switch (select) {
                            case 0 -> ChangeMapService.gI().changeMap(player, 48, -1, 354, 240);
                            case 1 -> Service.gI().sendThongBao(player, "Tạm thời không thể đến hành tinh Bill!");
                        }
                    }
                }
                case 154 -> {
                    if (player.iDMark.isBaseMenu()) {
                        switch (select) {
                            ...
                            case 1 -> ShopService.gI().opendShop(player, "SHOP_OSIN", false);
                        }
                    }
                }
                case 155 -> {
                ...
                case 114, 115, 117, 118, 119, 120 -> {
                    ...
                    switch (select) {
                        case 0 -> NpcService.gI().createTutorial(player, tempId, 4388, ConstNpc.HUONG_DAN_MAP_MA_BU);
                        case 1 -> {
                            if (!player.itemTime.isUseGTPT) {
