 ...
 */
import boss.BossID;
import boss.boss_manifest.Training.TauPayPay;
...
//            }
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (pl.isAdmin() || pl.isBoss || Util.canDoWithTime(pl.iDMark.getLastTimeChangeZone(), 15000)) {
            ...
            NpcService.gI().createTutorial(pl, -1, "Chưa thể chuyển khu vực lúc này vui lòng chờ "
                    + TimeUtil.getTimeLeft(pl.iDMark.getLastTimeChangeZone(), 15) + " nữa");
        }
    }
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            ...
            if (zoneJoin.map.mapId == 47) {
                ...
                if (TaskService.gI().getIdTask(pl) > ConstTask.TASK_9_0 && TaskService.gI().getIdTask(pl) < ConstTask.TASK_10_1) {
                    TrainingService.gI().callBoss(pl, BossID.TAUPAYPAY_BOMONG, false);
                } else if (TaskService.gI().getIdTask(pl) == ConstTask.TASK_10_1) {
