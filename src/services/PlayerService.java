...
public class PlayerService {
    ...
    public void playerMove(Player player, int x, int y) {
        ...
        if (!player.isDie()) {
            ...
            if (player.isPl()) {
                ...
                if (player.isFly) {
                    boolean hasOption84 = false; // bay không tốn KI
                    boolean hasOption85 = false; // hồi KI
                    boolean hasOption89 = false; // hồi HP + KI

                    if (player.getMount() != -1 && player.inventory.itemsBody.size() > 7) {
                        var mount = player.inventory.itemsBody.get(7);
                        if (mount != null && mount.isNotNullItem() && mount.itemOptions != null) {
                            for (var opt : mount.itemOptions) {
                    ...
                    if (hasOption85 || hasOption89) {
                        // Hồi mỗi giây 2%
                        if (Util.canDoWithTime(player.lastTimeAutoRecover, 1000)) {
                            player.lastTimeAutoRecover = System.currentTimeMillis();
                            int hpMax = player.nPoint.hpMax;
                            int mpMax = player.nPoint.mpMax;
                            long hpToAdd = hasOption89 ? hpMax * 2 / 100 : 0;
                            long mpToAdd = mpMax * 2 / 100;
                            hoiPhuc(player, hpToAdd, mpToAdd);
                        }
                    } else if (!hasOption84) {
                        // Không có opt 84 thì vẫn trừ KI như thường
    ...
    public void dailyLogin(Player player) {
        if (Util.compareDay(Date.from(Instant.now()), player.firstTimeLogin)) {
            ...
            player.drop_limit = new byte[player.drop_limit.length];
        }
    }
