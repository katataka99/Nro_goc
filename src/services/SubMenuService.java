...
//                    new CuuSat(player, plTarget);
                ...
                break;
            case BUY_BACK:
                ItemShop item = ItemService.gI().getItemShop(plTarget.inventory.itemsBody.get(5).template.id);
                if (item.cost <= 0) {
                    Service.gI().sendThongBao(player, "Không thể mua lại do giá vật phẩm không hợp lệ !!!");
                    return;
                }
                int cost = item.cost * 95 / 100;
                if (player.inventory.gem < cost) {
                    Service.gI().sendThongBao(player, "Bạn còn thiếu " + (cost - player.inventory.gem) + " ngọc xanh");
                    return;
                }
                NpcService.gI().createMenuConMeo(player, ConstNpc.BUY_BACK, -1,
                        "Bạn có muốn mua Cải trang thành " + item.temp.name + ", " + item.temp.description + " từ " + plTarget.name + " không?\n"
                        + "Giá cửa hàng " + item.cost + " ngọc xanh\n"
                        + "Giá tại đây là " + cost + " ngọc xanh\n"
                        + "Bạn cũng có thể nhận được " + (item.cost * 20 / 100) + " ngọc xanh\n"
                        + "khi người khac mua từ bạn(Cải trang của bạn sẽ không mất)", new String[]{"Đồng ý", "Từ chối"}, plTarget);
                break;
        }
        Service.gI().hideWaitDialog(player);
    }

    public void showMenu(Player player, Player plTarget) {
