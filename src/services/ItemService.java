...
//                break;
    ...
    public ItemMap randDoTuongLai(Zone zone, int quantity, int x, int y, long id) {
        // id item đồ
        int[][][] items = {
            {{137, 138, 136}, {153, 154, 152}, {169, 170, 168}}, // áo   / TĐ/NM/XD
            {{141, 142, 140}, {157, 158, 156}, {173, 174, 172}}, // quần
            {{149, 150, 144}, {165, 166, 164}, {181, 182, 180}}, // giày
            {{145, 146, 148}, {161, 162, 160}, {177, 178, 176}}, // găng
            {{185, 186, 184}, {185, 186, 184}, {185, 186, 184}} // rada
        };
        ...
        int itemId = items[type][Util.nextInt(items[type].length)][Util.nextInt(2)];
        List<ItemOption> options = ItemService.gI().getListOptionItemShop((short) itemId);
        for (ItemOption opt : options) {
            int originalValue = opt.param;
            // Tăng hoặc giảm từ -5% đến +5%
            int variation = (int) (originalValue * Util.nextInt(-5, 6) / 100.0);
            opt.param = Math.max(1, originalValue + variation); // Không cho nhỏ hơn 1
        }

        ItemMap it = new ItemMap(zone, itemId, quantity, x, y, id);
        it.options = options;
        // 5% item có option
    ...
    public ItemMap randDoNappa(Zone zone, int quantity, int x, int y, long id) {
        // id item đồ
        int[][][] items = {
            {{3, 34, 136}, {4, 42, 152}, {5, 50, 168}}, // áo   / TĐ/NM/XD
            {{9, 36, 140}, {10, 44, 156}, {11, 52, 172}}, // quần
            {{39, 40, 144}, {31, 48, 164}, {32, 56, 180}}, // giày
            {{24, 38, 148}, {25, 45, 160}, {26, 54, 176}}, // găng
            {{58, 59, 184}, {58, 59, 184}, {58, 59, 184}} // rada
        };
    ...
    // Muốn đồ thì code thêm vào đây
    public ItemMap randDoThuong(Zone zone, int quantity, int x, int y, long id) {
        // id item đồ
        int[][][] items = {
            {{0, 3, 33}, {1, 4, 41}, {2, 5, 49}}, // áo   / TĐ/NM/XD
            {{6, 9, 35}, {7, 10, 43}, {8, 11, 51}}, // quần
            {{27, 39, 30}, {28, 31, 47}, {29, 32, 55}}, // giày
            {{21, 24, 37}, {22, 25, 46}, {23, 26, 53}}, // găng
            {{12, 57, 58}, {12, 57, 58}, {12, 57, 58}} // rada
        };
    ...
    public Template.ItemTemplate getTemplate(int id) {
        for (Template.ItemTemplate t : Manager.ITEM_TEMPLATES) {
            if (t.id == id) {
                return t;
            }
        }
...
//    public void OpenItem1840(Player player, Item itemUse) {
//        try {
//            if (InventoryService.gI().getCountEmptyBag(player) <= 1) {
//                Service.gI().sendThongBao(player, "Bạn phải có ít nhất 2 ô trống hành trang");
//                return;
...
//                item = gokugod(true);
//            } else if (rd <= vp1 + vp2 + vp3 + vp4 + vp5 + vp6) {
//                item = gokussj2(true);
//            } else if (rd <= vp1 + vp2 + vp3 + vp4 + vp5 + vp6 + vp7) {
//                item = gokussj4red(true);
//            }
//            icon[0] = itemUse.template.iconID;
//            icon[1] = item.template.iconID;
//            InventoryService.gI().subQuantityItemsBag(player, itemUse, 1);
...
//        }
//    }
    public void OpenItem1840(Player pl, Item item) {
        if (InventoryService.gI().getCountEmptyBag(pl) > 1) {
            short[] temp = {1589, 1595, 1587, 1590, 1588};
            ...
            it.itemOptions.add(new Item.ItemOption(50, Util.nextInt(16, 27)));
            it.itemOptions.add(new Item.ItemOption(77, Util.nextInt(16, 27)));
            it.itemOptions.add(new Item.ItemOption(103, Util.nextInt(16, 27)));
    ...
    public Item gokussj3(boolean rating) {
        ...
        item.itemOptions.add(new Item.ItemOption(50, 23));//VIP
        item.itemOptions.add(new Item.ItemOption(77, 23));//hp 28%
        item.itemOptions.add(new Item.ItemOption(103, 23));//ki 25%
        item.itemOptions.add(new Item.ItemOption(210, 3));//stcm 22%
        if (Util.isTrue(995, 1000) && rating) {// tỉ lệ ra hsd
            item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(15) + 7));//hsd
        }
        return item;
    ...
    public Item gokussj(boolean rating) {
        Item item = createItemSetKichHoat(1589, 1);
        item.itemOptions.add(new Item.ItemOption(50, 21));//VIP
        item.itemOptions.add(new Item.ItemOption(77, 21));//hp 28%
        item.itemOptions.add(new Item.ItemOption(103, 21));//ki 25%
        item.itemOptions.add(new Item.ItemOption(210, 1));//stcm 22%
        if (Util.isTrue(995, 1000) && rating) {// tỉ lệ ra hsd
            item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(15) + 7));//hsd
        }
        return item;
    ...
    public Item goku(boolean rating) {
        Item item = createItemSetKichHoat(1588, 1);
        item.itemOptions.add(new Item.ItemOption(50, 23));//VIP
        item.itemOptions.add(new Item.ItemOption(77, 23));//hp 28%
        item.itemOptions.add(new Item.ItemOption(103, 23));//ki 25%
        item.itemOptions.add(new Item.ItemOption(210, 3));//stcm 22%
    ...
    public Item gokugod(boolean rating) {
        Item item = createItemSetKichHoat(1593, 1);
        item.itemOptions.add(new Item.ItemOption(50, 25));//VIP
        item.itemOptions.add(new Item.ItemOption(77, 25));//hp 28%
        item.itemOptions.add(new Item.ItemOption(103, 25));//ki 25%
        item.itemOptions.add(new Item.ItemOption(210, 4));//stcm 22%
        item.itemOptions.add(new Item.ItemOption(106, 0));//stcm 22%
        if (Util.isTrue(995, 1000) && rating) {// tỉ lệ ra hsd
            item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(15) + 7));//hsd
        }
        return item;
    ...
    public Item gokussj2(boolean rating) {
        Item item = createItemSetKichHoat(1595, 1);
        ...
        item.itemOptions.add(new Item.ItemOption(210, 2));//stcm 22%
        if (Util.isTrue(995, 1000) && rating) {// tỉ lệ ra hsd
            item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(15) + 7));//hsd
        }
        return item;
    ...
    public Item gokussj4red(boolean rating) {
        Item item = createItemSetKichHoat(1803, 1);
        item.itemOptions.add(new Item.ItemOption(50, 24));//VIP
        item.itemOptions.add(new Item.ItemOption(77, 24));//hp 28%
        item.itemOptions.add(new Item.ItemOption(103, 24));//ki 25%
        item.itemOptions.add(new Item.ItemOption(210, 3));//stcm 22%
    ...
    public Item gokublue(boolean rating) {
        ...
        item.itemOptions.add(new Item.ItemOption(50, 27));//VIP
        item.itemOptions.add(new Item.ItemOption(77, 27));//hp 28%
        item.itemOptions.add(new Item.ItemOption(103, 27));//ki 25%
        item.itemOptions.add(new Item.ItemOption(210, 5));//stcm 22%
        item.itemOptions.add(new Item.ItemOption(106, 0));//stcm 22%
        if (Util.isTrue(995, 1000) && rating) {// tỉ lệ ra hsd
            item.itemOptions.add(new Item.ItemOption(93, new Random().nextInt(15) + 7));//hsd
        }
    ...
    public ItemMap randDoTL(Zone zone, int quantity, int x, int y, long id) {
        ...
        int rand = Util.nextInt(100); // random từ 0 đến 99

        if (rand < 5) { // 0–4
            idTempTL = nhan[0];
            type = 4; // nhẫn (rada)
        } else if (rand < 15) { // 5–14
            idTempTL = gang[Util.nextInt(3)];
            type = 2; // găng
        } else if (rand < 35) { // 15–34
            idTempTL = quan[Util.nextInt(3)];
            type = 1; // quần
        } else if (rand < 60) { // 35–59
    ...
    public ItemMap randDo(Zone zone, int quantity, int x, int y, long id) {
        short idTempTL, type;
        short[] ao = {555, 557, 559};
        short[] quan = {556, 558, 560};
        short[] gang = {562, 564, 566};
    ...
    public ItemShop getItemShop(short id) {
        for (Shop shop : Manager.SHOPS) {
            for (TabShop t : shop.tabShops) {
                for (ItemShop it : t.itemShops) {
                    if (it.temp.id == id) {
                        return it;
                    }
                }
