        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_9_0:
                    ...
                    TrainingService.gI().callBoss(player, BossID.TAUPAYPAY_BOMONG, false);
                    break;
                ...
                case ConstTask.TASK_22_1:
                    Service.gI().sendThongBao(player, "Tiếp theo là Số 2");
                    break;
                ...
                case ConstTask.TASK_30_6:
                    ...
                    break;
                case ConstTask.TASK_30_7:
                    npcSay(player, ConstNpc.OSIN, "Tìm nhẫn thời không từ Black Goku");
                    break;
                case ConstTask.TASK_31_2:
                    ...
                    Service.gI().chatJustForMe(player, player, "OK cứ để tôi lo");
                    break;
                case ConstTask.TASK_31_4:
                    ...
                    Service.gI().chatJustForMe(player, player, "Chuyện nhỏ, không thành vấn đề");
                    break;
                case ConstTask.TASK_31_5:
                    npcSay(player, ConstNpc.BERRY, "Cậu sẽ đưa tôi về chỗ Bardock thật sao ?");
                    Service.gI().chatJustForMe(player, player, "Mau leo lên lưng tôi");
                    break;
                case ConstTask.TASK_31_6:
                    npcSay(player, ConstNpc.BARDOCK,
                            "Mơn cậu lần nữa\nHiện tại trong hang không còn gì để ăn\nCậu có thể giúp tôi tìm một ít lương thực được không ?");
                    Service.gI().chatJustForMe(player, player, "Yên tâm, tôi tìm giúp cho");
                    break;
                case ConstTask.TASK_31_3:
                case ConstTask.TASK_31_8:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn đánh được "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount);
                    ...
                    break;
                case ConstTask.TASK_31_7:
                    if (isCurrentTask(player, idTaskCustom)) {
                        Service.gI().sendThongBao(player, "Bạn nhặt được "
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).count + "/"
                                + player.playerTask.taskMain.subTasks.get(player.playerTask.taskMain.index).maxCount
                                + " Thức Ăn");
                    }
                    break;
                case ConstTask.TASK_31_9:
                    npcSay(player, ConstNpc.BARDOCK,
                            "Chờ Update Thêm Nhé !!!");
            }
        ...
        if (player.playerTask.sideTask.template != null) {
            if (player.playerTask.sideTask.isDone()) {
                ...
                if (InventoryService.gI().getCountEmptyBag(player) > 1) {
                    ...
                    Logger.success("Người chs: " + player.name + "Vàng nhận từ NV Bò mộng: " + goldReward);
                    Service.gI().sendMoney(player);
                    Service.gI().sendThongBao(player, "Bạn nhận được "
                            + Util.numberToMoney(goldReward) + " vàng");
                    player.effect.addPointNongDanChamChi(); // Thêm điểm sau mỗi nhiệm vụ hoàn thành
                    player.playerTask.sideTask.reset();
                } else {
