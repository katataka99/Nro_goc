...
//        sendPhanThab(player, plTarget, mobTarget);
            ...
            case Skill.TU_SAT:
                if (!player.playerSkill.prepareTuSat) {
                    ...
                    if (player.setClothes.cadicM == 2) {
                        rangeBom = SkillUtil.getRangeBom(player.playerSkill.skillSelect.point) + 400;
                    }
                    if (player.setClothes.cadicM == 4) {
                        dame += player.nPoint.hpMax * 20 / 100;//20%
                    } else if (player.setClothes.cadicM == 5) {
                        dame += player.nPoint.hpMax * 50 / 100;//50%
                    }
                    ...
                    // *** áp dụng tuyệt kỹ +#% sát thương (216) ***
                    if (player.nPoint.tlDameDacBiet != null && !player.nPoint.tlDameDacBiet.isEmpty()) {
                        for (Integer tile : player.nPoint.tlDameDacBiet) {
                            dame += dame * tile / 100;
                        }
                        if (!player.isBoss) {
                            for (Mob mob : player.zone.mobs) {
                    ...
                    if (!MapService.gI().isMapOffline(player.zone.map.mapId)) {
                        for (Player pl : playersMap) {
                            if (!player.equals(pl) && canAttackPlayer(player, pl)
                                ...
                                dame = pl.isBoss ? player.effectSkill.isMonkey ? dame / 2 : dame / 2 : dame;
                                //khi chia 2, danh boss chia 2 
    ...
    public void phanSatThuong(Player plAtt, Player plTarget, long dame) {
        //plAtt: người bị pst
        //plTarget: người pst
//       System.err.println(plAtt.name);
        if (plAtt != null) {
            int percentPST = plTarget.nPoint.tlPST;
            if (percentPST != 0) {
                int damePST = (int) (long) (dame * percentPST / 100);
                Message msg = null;
                try {
                    msg = new Message(56);
                    ...
                    if (plAtt.isBoss && !(plAtt instanceof Broly || plAtt instanceof SuperBroly)) {
                        if (damePST > plAtt.nPoint.hpMax / 100) {
                            ...
                            damePST = (plAtt.nPoint.hpMax / 100 - giamdame) * (percentPST / 100);
                        }
                    }
 ...
 private void playerAttackMob(Player plAtt, Mob mob, boolean miss, boolean dieWhenHpFull) { 
    ...
    if (!mob.isDie() && plAtt != null && plAtt.nPoint != null && plAtt.playerSkill != null) {
        ...
        // ❌ VẤN ĐỀ 1: Bot không có effectSkin, có thể gây NullPointerException
        if (plAtt.isPl() && plAtt.effectSkin != null && plAtt.effectSkin.isXDame) {
            plAtt.effectSkin.isXDame = false;
        }
        
        // ❌ VẤN ĐỀ 2: Kiểm tra charms - Bot không có charms
        if ((plAtt.charms != null && plAtt.charms.tdBatTu > System.currentTimeMillis()
                || plAtt.effectSkill != null) && plAtt.nPoint.hp <= 1) {
            if (plAtt.nPoint.hp < 1) {
                plAtt.nPoint.hp = 1;
            }
            if (!plAtt.isPet) {
                dameHit = 0;
                // ❌ VẤN ĐỀ 3: Bot không có session, sendThongBao có thể gây lỗi
                Service.gI().sendThongBao(plAtt, "Bạn đang được bùa bất tử bảo vệ không thể tấn công!"); 
            }
        }
        
        // Kiểm tra charms mạnh mẽ
        ...
        // ❌ VẤN ĐỀ 4: hutHPMP có thể cần session
        hutHPMP(plAtt, dameHit, null, mob);
        
        // ❌ VẤN ĐỀ 5: sendPlayerAttackMob có thể cần session
        sendPlayerAttackMob(plAtt, mob);
        
        // Gây sát thương cho mob
        mob.injured(plAtt, dameHit, dieWhenHpFull);
         
    } else {
        System.out.println("[DEBUG] Attack failed - conditions not met:");
        if (mob.isDie()) System.out.println("  - Mob is dead");
        if (plAtt == null) System.out.println("  - Player is null");
        if (plAtt != null && plAtt.nPoint == null) System.out.println("  - nPoint is null");
        if (plAtt != null && plAtt.playerSkill == null) System.out.println("  - playerSkill is null");
    }
}
