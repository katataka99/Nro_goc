...
public class PVPService {
                            ...
                            if (Client.gI().getPlayer(playerId) == null) {
                                if (plMap != null && plMap.isBoss) {
                                    NpcService.gI().createMenuConMeo(player, ConstNpc.IGNORE_MENU,
                                            -1, plMap.name + " (sức mạnh " + Util.numberToMoney(plMap.nPoint.TongTn())
    ...
    public void sendInvitePVP(Player pl, byte selectGold) {
        ...
        try {
            ...
            msg.writer().writeUTF(pl.name + " (sức mạnh " + Util.numberToMoney(pl.nPoint.TongTn()) + ") muốn thách đấu bạn với mức cược " + goldThachDau);
...
//        }
        ...
        try {
            ...
            msg.writer().writeUTF(pl.name + " (sức mạnh " + Util.numberToMoney(pl.nPoint.TongTn()) + ") muốn luyện tập với bạn");
            plMap.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
    ...
    public void acceptRevenge(Player pl) {
        ...
        if ((mapGo = ChangeMapService.gI().checkMapCanJoin(pl, mapGo)) == null || mapGo.isFullPlayer() || MapService.gI().isMapBanDoKhoBau(mapGo.map.mapId)
                || MapService.gI().isHome(mapGo.map.mapId) || MapService.gI().isMapBossFinal(mapGo.map.mapId) || MapService.gI().isMapKhiGasHuyDiet(mapGo.map.mapId)
                || MapService.gI().isMapDoanhTrai(mapGo.map.mapId) || MapService.gI().isMapSieuThanhThuy(mapGo.map.mapId) || MapService.gI().isMapConDuongRanDoc(mapGo.map.mapId)) {
            Service.gI().hideWaitDialog(pl);
            Service.gI().sendThongBao(pl, "Chưa thể đến lúc này, vui lòng thử lại sau ít phút");
            return;
        }
