...
public class RedRibbonHQ implements Runnable {
    ...
    private void init() {
        ...
        for (Zone zone : this.zones) {
            for (Mob mob : zone.mobs) {
                ...
                mob.point.dame = (int) Math.min((totalHp / mobTempId) / 3, 2_000_000_000L);      
                mob.point.maxHp = (int) Math.min((totalDamage * mobTempId) / 3, 2_000_000_000L); 
                mob.lvMob = 0;
                mob.hoiSinh();
                mob.hoiSinhMobPhoBan();
            }

            long dame = totalHp / 100;
            long hp = totalDamage * 10;

            if (zone.map.mapId == 59) {
                try {
                    long bossDamage = Math.min(dame, 50_000L);
                    long bossMaxHealth = Math.min(hp, 20_000_000L);
            ...
            if (zone.map.mapId == 62) {
                try {
                    long bossDamage = Math.min((long) (dame * 1.1), 50_000L);
                    long bossMaxHealth = Math.min((long) (hp * 1.1), 40_000_000L);
            ...
            if (zone.map.mapId == 55) {
                try {
                    long bossDamage = Math.min((long) (dame * 1.15), 70_000L);
                    long bossMaxHealth = Math.min((long) (hp * 1.15), 100_000_000L);
            ...
            if (zone.map.mapId == 54) {
                try {
                    long bossDamage = Math.min((long) (dame * 1.2), 100_000L);
                    long bossMaxHealth = Math.min((long) (hp * 1.2), 200_000_000L);
            ...
            if (zone.map.mapId == 57) {
                try {
                    long bossDamage = Math.min((long) (dame * 1.3), 120_000L);
                    long bossMaxHealth = Math.min((long) (hp * 1.3), 300_000_000L);
                    for (int i = 0; i < 4; i++) {
    ...
    int nr = Util.isTrue(1, 400) ? Util.nextInt(17, 20) : Util.nextInt(18, 20); // Giảm tỉ lệ ra đồ "hiếm"
    return new ItemMap(zone, nr, 1, x, y, -1);
}

public void randomNR() {
    for (Zone zone : zones) {
        if (Util.isTrue(1, 2)) { // từ 100% giảm còn 50%
            Service.gI().dropItemMap(zone, NR(zone));
        }
        if (Util.isTrue(1, 4)) { // từ 50% giảm còn 25%
            Service.gI().dropItemMap(zone, NR(zone));
        }
    }
}
    ...
    public void updateHPDame() {
        ...
        for (Zone zone : this.zones) {
            for (Mob mob : zone.mobs) {
                ...
                mob.point.dame = (int) Math.min((totalHp / mob.tempId) / 2, 2_000_000_000);
                mob.point.maxHp = (int) Math.min((totalDame * mob.tempId) / 2, 2_000_000_000);
