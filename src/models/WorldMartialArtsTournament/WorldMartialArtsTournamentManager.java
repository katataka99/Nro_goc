...
public class WorldMartialArtsTournamentManager implements Runnable {
    ...
    public void reward(Player pl) {
        ...
        // ✅ Thưởng vàng
        int rewardGold = 10_000_000; // 500 triệu vàng
        pl.inventory.gold += rewardGold;
        Logger.success("Người chs: " + pl.name + "Vàng nhận từ Đại hội võ thuật: " + rewardGold);
        Service.gI().sendThongBao(pl, "Bạn vừa nhận " + Util.numberToMoney(rewardGold) + " vàng");

        // ✅ Thưởng ngọc xanh
        int rewardGem = 10; // ví dụ: 50 ngọc xanh
        pl.inventory.gem += rewardGem;
        Service.gI().sendThongBao(pl, "Bạn vừa nhận " + rewardGem + " ngọc xanh");

        // ✅ Item danh dự ID 220 → 224 (mỗi loại 1 cái)
        ...
        String msg = ConstTournament.TEXT_KHOE_VO_DICH
                .replaceAll("%1", pl.name)
                .replaceAll("%2", ConstTournament.tournamentNames[WorldMartialArtsTournamentService.getTournament()]);
