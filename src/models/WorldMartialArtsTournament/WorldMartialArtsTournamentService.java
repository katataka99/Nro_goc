...
public class WorldMartialArtsTournamentService extends ConstTournament {
    ...
    public static String sayText() {
        return WorldMartialArtsTournamentManager.gI().canReg
                ...
                + tournamentNames[getTournament()]
                + ", đang có " + WorldMartialArtsTournamentManager.gI().listReg.size() + " người đăng ký thi đấu"
        ...
        switch (player.iDMark.getIndexMenu()) {
            ...
            case ConstNpc.BASE_MENU -> {
                switch (select) {
                    ...
                    case 1 -> {
                        if (canReg && tour != -1) {
                            int tourId = getTournament();
                            long playerPower = player.nPoint.power;

                            if ((tournamentPower[tourId] != 0 && playerPower > tournamentPower[tourId])
                                    || playerPower < minTournamentPower[tourId]) {
                                NpcService.gI().createTutorial(player, npc.tempId, npc.avartar, "Sức mạnh của bạn không phù hợp với giải đấu này");
                                return;
                            }
                            if (!regCheck(player)) {
                                ArrayList<String> menu = new ArrayList<>();
