...
public class ItemMap {
    ...
    public final int TIME_VETINH = 1_800_000;
    ...
    public void update() {
        if (isNotNullItem()) {
            ...
            if (((Util.canDoWithTime(createTime, 50000) && isNotNullItem() && itemTemplate.type != 22) || (Util.canDoWithTime(createTime, 180000) && itemTemplate.type != 22)) && !this.isNamecBall) {
            ...
            if (!pl.isDie() && Util.getDistance(pl.location.x, pl.location.y, x, y) < 200 && pl.satellite != null && (pl.id == this.playerId || this.clanId != -1 && pl.clan != null && pl.clan.id == this.clanId)) {
                switch (this.itemTemplate.id) {
                    case 342 -> {
                        ...
                        if (Util.canDoWithTime(pl.satellite.lastMPTime, TIME_VETINH)) {
                            ItemMapService.gI().removeItemMapAndSendClient(this);
                        }
                    }
                    case 343 -> {
                        ...
                        if (Util.canDoWithTime(pl.satellite.lastIntelligentTime, TIME_VETINH)) {
                            ItemMapService.gI().removeItemMapAndSendClient(this);
                        }
                    }
                    case 344 -> {
                        ...
                        if (Util.canDoWithTime(pl.satellite.lastDefendTime, TIME_VETINH)) {
                            ItemMapService.gI().removeItemMapAndSendClient(this);
                        }
                    }
                    case 345 -> {
                        ...
                        if (Util.canDoWithTime(pl.satellite.lastHPTime, TIME_VETINH)) {
                            ItemMapService.gI().removeItemMapAndSendClient(this);
                        }
                    }
                }
