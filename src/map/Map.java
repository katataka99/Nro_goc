    ...
    public Zone getMinPlayerZone() {
        ...
        synchronized (lock) {
            for (Zone z : this.zones) {
                if (max > z.getNumOfPlayers()) {
                    zone = z;
                    max = z.getNumOfPlayers();
                }
            }
        }
        ...
        return zones.get(Util.nextInt(zones.size())).zoneId;
    }
...
public Zone getZoneByID(int id) {
  ...
  public int tileTypeAtPixel(int px, int py) {
    ...
    // Ngoài map
    if (tileX < 0 || tileX >= tmw || tileY < 0) {
        return 1000;
    }

    int index = tileY * tmw + tileX;
    ...
    public final void readTileMap(int mapId) {
        try {
            FileInputStream f = new FileInputStream("data/map/tile_map_data/" + mapId);
            ...
            this.blocks = new boolean[maps.length];
