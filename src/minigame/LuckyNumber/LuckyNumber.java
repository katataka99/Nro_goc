...
public class LuckyNumber implements Runnable {
    ...
    public static void showMenu(Npc npc, Player player, boolean isGem) {
        if (!isOpen()) {
            npc.createOtherMenu(player, LyTieuNuong.ConstMiniGame.MENU_LUCKY_NUMBER,
                    "Thời gian từ 8h sáng đến hết 24h hằng ngày\n"
                    + "Mỗi lượt được chọn 10 con số từ 0 đến 99\n"
                    + "Thời gian mỗi lượt là 3 phút.",
                    "Cập nhật", "Đóng");
        } else {
    ...
    public static void spinGame() {
        ...
        spinGame = false;
        players.forEach((g) -> {
            Player player = SQLHandle.loadById(g.id);
            if (player != null) {
    ...
    public static void checkPlayerWin(long id) {
        ...
        ServerNotify.gI().notify("Con số chúng thưởng là " + RESULT + " chúc bạn may mắn lần sau");
        for (LuckNumberData g : players) {
            ...
            if (id == g.id && g.number == RESULT && !g.isReward && player != null) {
                if (g.isGem) {
                    ...
                    ServerNotify.gI().notify("Chúc mừng " + player.name + " đã thắng " + LuckyNumberCost.costGem + " ngọc với con số may mắn " + RESULT);
                    ...
                    ServerNotify.gI().notify("Chúc mừng " + player.name + " đã thắng " + Util.mumberToLouis(LuckyNumberCost.costGold) + " vàng với con số may mắn " + RESULT);
                    LuckyNumberService.addDataResul(player, g.number, LuckyNumberCost.costGold, finish);
                    g.isReward = true;
                }
                ServerNotify.gI().notify("Người trúng giải: " + player.name);
                DATA_PLAYER_RESULT.add("Người trúng giải: " + player.name);
...
//                        System.out.println("Trao ngọc: " + player.name);
//                    } else {
