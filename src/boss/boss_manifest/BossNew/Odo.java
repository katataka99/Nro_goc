package boss.boss_manifest.BossNew;
import boss.Boss;
...
public class Odo extends Boss{
    ...
    public Odo() throws Exception {
        super(BossID.O_DO_2, new BossData(
   ...
   private void updateOdo() {
        ...
        if (Util.canDoWithTime(lastTimeOdo, randomTime)) {
            ...
            for (int i = playersMap.size() - 1; i >= 0; i--) {
                ...
                if (pl != null && pl.nPoint != null && !this.equals(pl) && !pl.isBoss && !pl.isDie()
                    ...
                    this.chat( "Bùm Bùm");
                    services.PlayerService.gI() .sendInfoHpMpMoney(pl);
    ...
    private void regenHp() {
        try {
            if (Util.canDoWithTime(lastTimeHpRegen, 30000)) {
                ...
                services.PlayerService.gI().hoiPhuc(this, regenAmount, 0);
    ...
    public void moveTo(int x, int y) {
        ...
        services.PlayerService.gI().playerMove(this, this.location.x + (dir == 1 ? move : -move), y);
    }
...
public void reward(Player plKill) {
    ...
    // Tỉ lệ rơi item id 77 (ví dụ 30%)
    if (Util.isTrue(30, 100)) {
        int soLuong = Util.nextInt(5, 10); // random từ 5 đến 10
        ItemMap item77 = new ItemMap(this.zone, 77, soLuong,
                this.location.x,
    ...
    private static class PlayerService {
