...
public class TrungUyTrang extends Boss {
    ...
    public void reward(Player plKill) {
        ...
        if (Util.isTrue(50, 100)) {
            ItemMap it611 = new ItemMap(this.zone, 611, 1, this.location.x,
                    this.zone.map.yPhysicInTop(this.location.x, this.location.y - 24), plKill.id);
            Service.gI().dropItemMap(this.zone, it611);
        }
    }
    ...
    public void die(Player plKill) {
        ...
        // Hoàn thành nhiệm vụ cho tất cả người chơi trong cùng zone 
        for (Player pl : this.zone.getPlayers()) {
            if (pl != null && !pl.isDie()) {
                TaskService.gI().doneTask(pl, ConstTask.TASK_19_1);
            }
        }
    }
