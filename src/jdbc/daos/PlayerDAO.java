...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.add(15000000); // tiềm năng
            ...
            DBConnecter.executeUpdate("insert into player"
                    ...
                    + "items_bag, items_box,items_box_collection, items_box_lucky_round, items_daban, friends, enemies, data_intrinsic, data_item_time,"
                    + "data_task, data_mabu_egg, data_charm, skills, skills_shortcut, pet,"
                    + "data_black_ball, data_side_task,data_kol_task, dataBadges, dataTaskBadges, dailyGift,item_mails_box,BoughtSkill) "
                    + "values ()", userId, name, hair, gender, 0, -1, inventory, location, point, magicTree,
                    itemsBody, itemsBag, itemsBox, itemsBoxCollection, itemsBoxLuckyRound, itemsDaBan, friends, enemies, intrinsic,
                    itemTime, task, mabuEgg, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, koltask,
                    dataBadges, dataTaskBadges, dailyGift, itemsMailBox, dataBoughtSkill);
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                dataArray.add(player.gemChallenge);
                ...
                tet.add(player.point_trung);
                tet.add(player.point_gioto);
                tet.add(player.point_banhday);
                ...
                JSONArray dataDropLimit = new JSONArray();
                for (int i = 0; i < player.drop_limit.length; i++) {
                    dataDropLimit.add(player.drop_limit[i]);
                }
                String dropLimit = dataDropLimit.toJSONString();

                // data box mail
                for (Item item : player.inventory.itemsMailBox) {// Zalo: 0866468126//Name: EMTUAANS
                    JSONArray opt = new JSONArray();
                    if (item.isNotNullItem()) {// Zalo: 0866468126//Name: EMTUAANS
                        dataItem.add(item.template.id);
                        dataItem.add(item.quantity);
                        JSONArray options = new JSONArray();
                        for (Item.ItemOption io : item.itemOptions) {// Zalo: 0866468126//Name: EMTUAANS
                            opt.add(io.optionTemplate.id);
                            opt.add(io.param);
                            options.add(opt.toJSONString());
                            opt.clear();
                        ...
                        dataItem.add(options.toJSONString());
                    } else {// Zalo: 0866468126//Name: EMTUAANS
                        dataItem.add(-1);
                        dataItem.add(0);
                        dataItem.add(opt.toJSONString());
                    }
                ...
                String query = "update player set head = ?, have_tennis_space_ship = ?, "
                        ...
                        + "items_body = ?, items_bag = ?, items_box = ?,items_box_collection= ?, items_box_lucky_round = ?, items_daban = ?, friends = ?, "
                        ...
                        + " power = ?,data_tet=?,danh_hieu_shop = ?,item_mails_box=?, LearnSkill = ?, BoughtSkill = ?, drop_limit=? where id = ?";
    ...
    public static boolean addCarot(Player player, int num) {
        String updateQuery = "UPDATE account SET carot = carot + ? WHERE id = ?";
        try ( Connection con = DBConnecter.getConnectionServer();  PreparedStatement ps = con.prepareStatement(updateQuery)) {
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().userId);
            int rowsAffected = ps.executeUpdate();
            if (rowsAffected > 0) {
                player.getSession().carot += num;
                return true;
            } else {
                return false;
            }
