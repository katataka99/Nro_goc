...
public class ShopService {
                ...
                for (TabShop tab : shop.tabShops) {
                    ...
                    for (ItemShop itemShop : tab.itemShops) {
                        ...
                        var costPotential = SkillUtil.findSkillTemplate(skillTemplateId).skillss.stream()
                                ...
                                .findFirst()
                                .map(s -> s.powRequire) // Ép kiểu Long -> int
    ...
    private void openShopType4(Player player, String tagName, List<Item> items) {
        ...
        try {
            ...
            for (Item item : items) {
                ...
                msg.writer().writeUTF("Ngọc Rồng Sơ Cấp");
    ...
    private void buyItemShopGiamGia(Player player, int itemTempId) {
        Shop shop = player.iDMark.getShopOpen();
        ItemShop is = shop.getItemShop(itemTempId);
        if (is == null) {
            Service.gI().sendThongBao(player, "Không thể thực hiện");
        ...
        InventoryService.gI().addItemBag(player, ItemService.gI().createItemFromItemShop(is));
        InventoryService.gI().subQuantityItemsBag(player, InventoryService.gI().findItemBag(player, 459), itemTempId);
        InventoryService.gI().sendItemBag(player);
        opendShop(player, shop.tagName, true);
    }
    ...
    private void learnKyNang(Player pl, ItemShop is) {
        ...
        // Lấy skill template và tiềm năng yêu cầu thực sự
        ...
        long potential = SkillUtil.findSkillTemplate(skillTemplateId).skillss.stream()
                ...
                .map(s -> s.powRequire)
                .orElse(0l);
        // ✅ Kiểm tra đúng tiềm năng cần thiết
        if (pl.nPoint.tiemNang < potential) {
            Service.gI().sendThongBao(pl, "Bạn không đủ tiềm năng để học kỹ năng này");
            return;
        }

        // Xử lý thời gian học
        long[] time = new long[]{900000, 1800000, 3600000, 43200000, 86400000, 259200000, 475200000};
        ...
        // Gửi menu xác nhận học
        String text = "Con có muốn học kỹ năng "
                + SkillUtil.findSkillTemplate(skillTemplateId).name + " cấp " + level
        ...
        String[] menus = new String[]{"Yes", "No"};
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        // }
        if (is.tabShop.id == 38) {
            buyItemShopGiamGia(player, itemTempId);
            return;
        }
        if (is.tabShop.id == 28) {
            buyDanhHieu(player);
        ...
        Item trungVang = ItemService.gI().createItemFromItemShop(is);
        if (trungVang.template.id == 1821) {
            item = ItemService.gI().createNewItem((short) 1821);
            item.itemOptions.addAll(is.options);
            player.point_trung++;
        }
        Item capsule = ItemService.gI().createItemFromItemShop(is);
        if (capsule.template.id == 193) {
    ...
    private Item buyMagicPean(Player player, int[][] listDauThan, Item item) {
        for (int i = 0; i < listDauThan.length; i++) {
            if (item.template.id == listDauThan[i][1]) {
                ...
                item.itemOptions.add(new Item.ItemOption(2, 16));
                item.quantity = 30;
                return item;
            }
        }
    ...
    public void sellItem(Player pl, int where, int index) {
        ...
        if (item != null) {
            ...
            Logger.success("Người chs: " + pl.name + "Vàng nhận từ Bán item: "
                    + (cost));
