...
public class EffectSkin {
    ...
    private static final String[] textltdb = new String[]{
        "Ngọc Rồng Sơ Cấp", "Idol Vip Quá!", "Idol",
        "Trùm Server Đây Rồi!"
    };
...
//            updateHalloween();
    ...
    try {
        if (this.player.nPoint != null && this.player.nPoint.isXinbato) { // điều kiện đang mặc Xinbatô
            if (Util.canDoWithTime(lastTimeXinbato, 5000)) { // 5s một lần
                ...
                for (Player pl : playersMap) {
                    if (pl != null && pl.nPoint != null && !this.player.equals(pl)
                        ...
                        // Tăng tỉ lệ né đòn (làm đối thủ đánh hụt)
                        pl.nPoint.tlNeDon += 50; // tăng 30%
                    }
                }
