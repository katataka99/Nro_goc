...
public class NPoint {
    ...
    public static final byte MAX_LIMIT = 4;
    ...
    public NPoint(Player player) {
        ...
        this.tlDameDacBiet = new ArrayList<>();
    }
     ...
     */
    public List<Integer> tlDame, tlDameAttMob, tlDameDacBiet;
    ...
    private void setPointWhenWearClothes() {
        ...
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
            this.tlDame.add(RewardBlackBall.R1S_1);
        }
        if (this.player.rewardBlackBall.timeOutOfDateReward[1] > System.currentTimeMillis()) {
            this.tlHp.add(RewardBlackBall.R2S_1);
            this.tlMp.add(RewardBlackBall.R2S_1);
        }
                ...
                if (io.active == card.Level || (card.Level == -1 && io.active == 0)) {
                    switch (io.id) {
                        ...
                        case 5: // +#% sức đánh chí mạng
                            ...
                            break;
                        case 216:
                            this.tlDameDacBiet.add(io.param);
                            break;
                        case 6: // HP+#
                            this.hpAdd += io.param;
                            break;
...
//                            this.tlTNSM.add(io.param);
        ...
        for (Item item : this.player.inventory.itemsBody) {
            if (item.isNotNullItem()) {
                ...
                if (item.template.id >= 601 && item.template.id <= 603 || item.template.id >= 639 && item.template.id <= 641) {
                    if (player.fusion != null && player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
...
//                this.tlTNSM.add(io.param);
    ...
    private void setHpMax() {
        ...
        if (this.player != null && this.player.isAttackedByPlayerOrBoss(10_000)) {
            if (this.tl222 >= 0) {
                hpMax += (hpMax * this.tl222 / 100);
                System.out.println("log add hp");
            }
        }
...
//        }
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.TU_SAT:
                ...
                for (Integer tl : this.tlDameDacBiet) {
                    percentXDame += (percentXDame * tl / 100L);
                }
                break;
            ...
            case Skill.DICH_CHUYEN_TUC_THOI:
                isCrit = true;
                isCritTele = true;
                dameAttack = Util.nextLong(Math.min(2_000_000_000L, (dameAttack - (dameAttack / 100 * 5))),
                        Math.min(2_000_000_000L, (dameAttack + (dameAttack / 100 * 5))));
                break;
            case Skill.MAKANKOSAPPO:
                percentDameSkill = skillSelect.damage;
                int dameSkill = (int) Math.min(2_000_000_000L, this.mpMax * percentDameSkill / 100 * 1.3);
                for (Integer tl : this.tlDameDacBiet) {
                    dameSkill += (dameSkill * tl / 100L);
                }
                return dameSkill;
        ...
        if (dameAttack < 0) {
            System.err.println("âm: " + dameAttack);
            dameAttack = 0;
        }
    ...
    public void subHP(long sub) {
        ...
        if (this.hp <= 0) {
            this.hp = 0;
            this.setHp(0);
            if (this.player != null && this.player.zone != null && this.player.nPoint != null && this.player.zone.map != null && MapService.gI().isMapTuongLai(this.player.zone.map.mapId)) {
                this.player.nPoint.power -= this.player.nPoint.power * 2 / 10000 > 0 ? this.player.nPoint.power * 2 / 10000 : 0;
                PlayerService.gI().sendTNSM(player, (byte) 3, this.player.nPoint.power * 2 / 10000 > 0 ? this.player.nPoint.power * 2 / 10000 : 0);
            }
        }
    }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        if (player != null && player.zone.map.type == 3) {
            return 0;
        }
        if (power < getPowerLimit()) {
            ...
            if (player.zone.map.mapId >= 53 && player.zone.map.mapId <= 62) {
                tiemNang += tn * 5;
            }
...
//        }
        if (type == 3) {
            ...
            for (int i = 0; i < point; i++) {
                int defAfter = this.defg + i;
                tiemNangUse += (2 * (defAfter + 5) / 2) * 100000;
            }
    ...
    public long TnHP() {
        long TiemNang = 0;
        long hpg = 0;
        for (long currentHpgValue = hpg; currentHpgValue < this.hpg; currentHpgValue++) {
            long costForOnePoint = (1 * (2 * (currentHpgValue + 1000) + Diem - 20)) / 2;
            TiemNang += costForOnePoint;
        }
        return TiemNang;
    }
    ...
    public long TnMP() {
        ...
        for (long currentMpgValue = mpg; currentMpgValue < this.mpg; currentMpgValue++) {
            long costForOnePoint = (1 * (2 * (currentMpgValue + 1000) + Diem - 20)) / 2;
            TiemNang += costForOnePoint;
        }
        return TiemNang;
    }
    ...
    public long TnDame() {
        ...
        for (long currentDamegValue = dameg; currentDamegValue < this.dameg; currentDamegValue++) {
            long costForOnePoint = (1 * (2 * currentDamegValue + 1 - 1)) / 2 * 100;
            TiemNang += costForOnePoint;
        }
        return TiemNang;
    }
    ...
    public long TnDef() {
        long TiemNang = 0;
        long defg = 0;
        for (long currentDefgValue = defg; currentDefgValue < this.defg; currentDefgValue++) {
            long costForOnePoint = (2 * (currentDefgValue + 5)) / 2 * 100000;
            TiemNang += costForOnePoint;
        }
        return TiemNang;
    }
    ...
    public long TnCrit() {
        ...
        long initialCostPerPoint = 50_000_000L;
        long currentPointCost = initialCostPerPoint;
        for (int i = 0; i < this.critg; i++) {
            TiemNang += currentPointCost;
            currentPointCost *= 5L;
        }
        return TiemNang;
    }
